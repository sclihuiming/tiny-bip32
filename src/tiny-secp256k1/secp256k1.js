...
const deterministicGenerate = require('./rfc6979');
...
function isPrivate(x) {
    if (!isScalar(x))
        return false;
    return (x.compare(ZERO32) > 0 && x.compare(EC_GROUP_ORDER) < 0 // > 0
    ); // < G
}
...
function verify(hash, q, signature, strict) {
    ...
    if (r.gtn(0) <= 0 /* || r.compareTo(n) >= 0 */)
        return false;
    if (s.gtn(0) <= 0 /* || s.compareTo(n) >= 0 */)
        return false;
